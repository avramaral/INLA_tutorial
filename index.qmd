...
\end{align*}
where $\gamma \in \mathbb{R}$ is the "degree of preferentiality", and all other quantities are defined as before.
...
\end{align*}
From the above formulation, notice that $\gamma(x)$ is approximated by a combination of unknown coefficients $\{\beta_k\}_k$ and basis functions $\{\phi_k(x)\}_k$. All remaining quantities are defined as before.
...
For this tutorial, we will use "radial basis" built using a compactly
supported Wendland ([Wendland, 2015](https://doi.org/10.1007/BF02123482)) function defined in space.

> Radial Basis (Wendland): let $h_k = \left(||x - a_k|| / b_k\right)$, such that $b_k \in \mathbb{R}$ and $a_k \in \mathbb{R}^2$, $\forall k$. Under this setting, $a_k$ is also known as a "node point," and, sometimes, $b_k$ can also be referred to as "bandwidth." Then,
> $$ \phi_k(x) = \begin{cases} (1 - h_k)^6 (35h_k^2 + 18h_k + 3) / 3 & h_k \in [0, 1] \\ 0   & \text{otherwise.} \end{cases} $$
