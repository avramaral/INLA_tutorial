...
In particular, if $y_{g, i}$ is the $i$-the element in group $g$, then $\text{Cov}(y_{g_1, i_1}, y_{g_2, i_2}) = (\text{cov. between groups } g_1 \text{ and } g_2) \times (\text{cov. between elements } i_1 \text{ and } i_2)$ ([Simpson, 2016](http://faculty.washington.edu/jonno/SISMIDmaterial/8-Groupedmodels.pdf)).
...
As in [Moraga (2019)](https://www.paulamoraga.com/book-geospatial/sec-geostatisticaldatatheory.html#stochastic-partial-differential-equation-approach),
...
And create the mesh with the `inla.mesh.2d()` function. A tool for mesh assessment is described [here](https://becarioprecario.bitbucket.io/spde-gitbook/ch-intro.html#sec:toolsmesh).
